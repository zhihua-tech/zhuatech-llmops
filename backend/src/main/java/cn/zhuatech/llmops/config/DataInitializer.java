...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(OperatingUnitRepository units,WorkRecordRepository records,ResourceRegisterRepository resources,ReviewRecordRepository reviews,UserRepository users,PasswordEncoder encoder){return args->{if(units.count()>0)return;
  ...
  resources.saveAll(List.of(new ResourceRegister("MODEL-GENERAL-L","通用大语言模型 L",knowledge,ResourceRegister.Status.RUNNING,99),new ResourceRegister("MODEL-FAST-S","轻量低时延模型 S",service,ResourceRegister.Status.RUNNING,98),new ResourceRegister("MODEL-EMBED-CN","中文向量模型 E",marketing,ResourceRegister.Status.ALARM,76)));
  reviews.saveAll(List.of(new ReviewRecord("EVAL-LLM-260801",a,"综合质量",120,3,ReviewRecord.Result.PENDING,"季衡"),new ReviewRecord("EVAL-LLM-260728",b,"业务效果",86,0,ReviewRecord.Result.PASSED,"顾清和"),new ReviewRecord("EVAL-LLM-260726",c,"内容安全",200,12,ReviewRecord.Result.FAILED,"季衡")));
  String demo=encoder.encode("Demo@2026"); users.saveAll(List.of(new UserAccount("operator",demo,"顾清和",UserAccount.Role.DOMAIN_USER,"LLM-KB"),new UserAccount("planner",demo,"季衡",UserAccount.Role.DOMAIN_OPERATOR,null),new UserAccount("quality",demo,"顾清",UserAccount.Role.QUALITY,null),new UserAccount("admin",encoder.encode("ZhuaTech@2026"),"系统管理员",UserAccount.Role.ADMIN,null)));};}
}
